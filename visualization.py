...
def microphone_update(audio_samples):
    ...
    if vol < config.MIN_VOLUME_THRESHOLD:
        ...
        remote_led.pixels = np.tile(0, (3, config.N_PIXELS))
        remote_led.update()
    else:
        ...
        if config.USE_GUI:
            ...
            r_curve.setData(y=remote_led.pixels[0])
            g_curve.setData(y=remote_led.pixels[1])
            b_curve.setData(y=remote_led.pixels[2])
    if config.USE_GUI:
        app.processEvents()
